    ...
    <div style={{ fontFamily: "'IBM Plex Sans', sans-serif", background: "#f0f2f5", minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center" }}>
      <div style={{ width: 320 }}>
        <div style={{ textAlign: "center", marginBottom: 28 }}>
          <div style={{ fontFamily: "'IBM Plex Mono'", fontSize: 15, letterSpacing: "0.2em", color: "#8b4513", marginBottom: 6 }}>TRUEFIT SKIM COAT</div>
          <div style={{ fontFamily: "'IBM Plex Mono'", fontSize: 20, letterSpacing: "0.12em", color: "#8b4513" }}>RECEIVABLES</div>
        ...
        <div style={{ background: "#ffffff", border: "1px solid #cccccc", borderRadius: 12, padding: 24, boxShadow: "0 4px 16px rgba(0,0,0,0.06)" }}>
            ...
            {["salesman", "admin"].map(m => (
              <button key={m} onClick={() => { setMode(m); setPassword(""); setError(""); }}
                style={{ flex: 1, padding: "7px 0", fontFamily: "'IBM Plex Mono'", fontSize: 16, letterSpacing: "0.1em", borderRadius: 5, border: "1px solid", transition: "all 0.15s",
                  ...
                  color: mode === m ? "#ffffff" : "#8b4513"
                }}>{m.toUpperCase()}</button>
            ))}
          </div>
          {mode === "salesman" && (
            <>
              <div style={{ marginBottom: 8, fontFamily: "'IBM Plex Mono'", fontSize: 15, letterSpacing: "0.12em", color: "#8b4513" }}>YOUR NAME</div>
              <select value={selectedId} onChange={e => setSelectedId(e.target.value)}
                style={{ width: "100%", background: "#f8fafc", border: "1px solid #cccccc", borderRadius: 6, padding: "10px 14px", color: "#8b4513", fontSize: 16, fontFamily: "'IBM Plex Sans'", marginBottom: 12 }}>
                <option value="">— Select —</option>
                {salesmen.map(s => <option key={s.id} value={s.id}>{s.name}</option>)}
              </select>
            </>
          )}
          <div style={{ marginBottom: 8, fontFamily: "'IBM Plex Mono'", fontSize: 15, letterSpacing: "0.12em", color: "#8b4513" }}>
          ...
          <button onClick={handleLogin}
            style={{ width: "100%", padding: "10px 0", background: "#8b4513", border: "none", borderRadius: 6, color: "#ffffff", fontSize: 16, fontFamily: "'IBM Plex Mono'", letterSpacing: "0.1em", transition: "background 0.15s" }}
