...
export default function ProfitabilityView({ salesmen }) {
  ...
  const fetchData = async () => {
    ...
    // Current outstanding per salesman (from bills)
    const { data: allBills } = await supabase
      .from("bills")
      .select("dealer_id, balance");
    ...
    const rows = salesmen.map(sm => {
      ...
      const interestCost = Math.round(outstanding * 0.01); // 1% per month = 12% pa
      ...
      const overdue60 = smOutBills
        .filter(b => ageDaysSimple(b) > 60)
      ...
      return { sm, sales, collections, salary, expTotal, outstanding, interestCost, totalCost, net,
               collectionEfficiency, overdue60Ratio, costPerRupee, smExpenses, salaryId: smSalary?.id };
    });
      const smSalary   = (salaries || []).find(s => s.salesman_id === sm.id);
      const smExpenses = (expenses || []).filter(e => e.salesman_id === sm.id);
  ...
  const downloadCSV = () => {
    const rows = [
      ["Salesman", "Sales", "Collections", "Outstanding", "Interest (1%)", "Salary", "Expenses", "Total Cost", "Net", "Collection Eff%", "60d+ Ratio%", "Cost/Rs Collected%"],
      ...
      ["TOTAL",
        ...
        data.reduce((s, r) => s + r.interestCost, 0),
...
<style>
  ...
  <table>
    <thead><tr>
      ...
      <th>SALES</th><th>COLLECTIONS</th><th>OUTSTANDING</th><th>INTEREST</th><th>SALARY</th><th>EXPENSES</th><th>TOTAL COST</th><th>NET</th><th>COLL EFF%</th><th>60d+%</th>
    </tr></thead>
    <tbody>
      ${rows}
      <tr class="total">
