...
export default function SalesmanView({ salesman }) {
  ...
  return (
    <div className="fade-in">
      ...
      {viewingPDF && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.7)", zIndex: 1000, display: "flex", flexDirection: "column" }}>
          <div style={{ background: "#1e1c2e", padding: "12px 16px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            <span style={{ fontFamily: "'IBM Plex Mono'", fontSize: 14, color: "#c8c4e0" }}>{viewingPDF.bill_no}</span>
            <button onClick={() => setViewingPDF(null)} style={{ padding: "6px 12px", fontFamily: "'IBM Plex Mono'", fontSize: 12, borderRadius: 6, border: "1px solid #444", background: "transparent", color: "#ccc", cursor: "pointer" }}>✕ Close</button>
          </div>
      ...
      {/* Salesman name */}
      <div style={{ fontFamily: "'IBM Plex Mono'", fontSize: 24, fontWeight: 700, color: "#334155", letterSpacing: "0.08em", marginBottom: 16, textTransform: "uppercase" }}>{salesman.name}</div>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 16, flexWrap: "wrap", gap: 8 }}>
        <div>
          <div style={{ fontFamily: "'IBM Plex Mono'", fontSize: 10, letterSpacing: "0.12em", color: "#888", marginBottom: 4 }}>TOTAL OUTSTANDING</div>
          <div style={{ fontFamily: "'IBM Plex Mono'", fontSize: 30, fontWeight: 500, color: totalOut > 0 ? "#ea580c" : "#166534" }}>{fmt(totalOut)}</div>
        </div>
        <div style={{ display: "flex", gap: 8 }}>
          <button onClick={() => generateAgeingReport(salesman.name, dealers)} style={{
            padding: "6px 12px", fontSize: 13, borderRadius: 4,
            border: "1px solid #e2e8f0", background: "#334155", color: "#ffffff",
            fontFamily: "'IBM Plex Mono'", letterSpacing: "0.08em"
          }}>⬇ PDF</button>
          <button onClick={() => generateAgeingExcel(salesman.name, dealers)} style={{
            padding: "6px 12px", fontSize: 13, borderRadius: 4,
            border: "1px solid #334155", background: "#ffffff", color: "#334155",
            fontFamily: "'IBM Plex Mono'", letterSpacing: "0.08em"
          }}>⬇ Excel</button>
          <button onClick={fetchDealers} style={{ padding: "6px 12px", fontSize: 14, borderRadius: 4, border: "1px solid #e2e8f0", background: "#ffffff", color: "#888" }}>&#8635;</button>
        </div>
      </div>
      ...
      {totalChq > 0 && (
        <div style={{ background: "#fffbeb", border: "1px solid #d4a820", borderRadius: 8, padding: "10px 14px", marginBottom: 14, display: "flex", alignItems: "center", gap: 8 }}>
          <span>🟡</span>
          <span style={{ fontFamily: "'IBM Plex Mono'", fontSize: 12, color: "#92640a", fontWeight: 600 }}>CHQ PENDING · {fmt(totalChq)}</span>
        </div>
      )}

      {/* Ageing buckets */}
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 10, marginBottom: 20 }}>
        {[{ label: "0-30 DAYS", val: b30, color: "#166534" }, { label: "31-60 DAYS", val: b60, color: "#7a5200" }, { label: "60+ DAYS", val: b60p, color: "#ea580c" }].map(s => (
          <div key={s.label} style={{ ...card, padding: "12px 14px" }}>
            <div style={{ fontFamily: "'IBM Plex Mono'", fontSize: 10, letterSpacing: "0.12em", color: "#888", marginBottom: 4 }}>{s.label}</div>
            <div style={{ fontFamily: "'IBM Plex Mono'", fontSize: 18, fontWeight: 500, color: s.color }}>{fmt(s.val)}</div>
          </div>
        ))}
      </div>

      <div style={{ fontFamily: "'IBM Plex Mono'", fontSize: 10, letterSpacing: "0.12em", color: "#888", marginBottom: 10 }}>DEALERS ({visibleDealers.length})</div>
      ...
      <div style={{ position: "relative", marginBottom: 16 }}>
        ...
        <input type="text" value={search} onChange={e => setSearch(e.target.value)} placeholder="Search dealer name..."
          style={{ width: "100%", padding: "10px 12px 10px 34px", border: "1px solid #e2e8f0", borderRadius: 8, fontSize: 15, background: "#ffffff", color: "#334155", outline: "none" }} />
        {search && <button onClick={() => setSearch("")} style={{ position: "absolute", right: 10, top: "50%", transform: "translateY(-50%)", border: "none", background: "none", cursor: "pointer", fontSize: 18, color: "#aaa" }}>✕</button>}
      </div>
              ...
              <div key={dealer.id} className="fade-in" style={{ ...card, overflow: "hidden", borderLeft: "3px solid " + sc, animationDelay: i * 40 + "ms" }}>
                <div style={{ padding: "14px 16px" }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 8, flexWrap: "wrap", gap: 8 }}>
                    ...
                    <div style={{ textAlign: "right" }}>
                      <div style={{ fontFamily: "'IBM Plex Mono'", fontSize: 18, fontWeight: 500, color: "#334155" }}>{fmt(bal)}</div>
                      <span style={{ fontSize: 12, padding: "2px 8px", borderRadius: 4, background: bucket.bg, color: bucket.color, border: "1px solid " + bucket.border }}>{bucket.label}</span>
                    </div>
                  </div>
                  ...
                  {pending.length > 0 && (
                    <div style={{ marginBottom: 8, display: "flex", flexDirection: "column", gap: 3 }}>
                      {pending.map(c => (
                        <span key={c.id} style={{ background: "#fffbeb", border: "1px solid #d4a820", borderRadius: 4, padding: "2px 8px", fontSize: 12, color: "#92640a", fontFamily: "'IBM Plex Mono'", display: "inline-block" }}>
                  ...
                  {bounced.length > 0 && (
                    <div style={{ marginBottom: 8, display: "flex", flexDirection: "column", gap: 3 }}>
                      {bounced.map(c => (
                        <span key={c.id} style={{ background: "#fff5f5", border: "1px solid #fecaca", borderRadius: 4, padding: "2px 8px", fontSize: 12, color: "#dc2626", fontFamily: "'IBM Plex Mono'", display: "inline-block" }}>
                  ...
                  <button onClick={() => toggle(dealer.id)}
                    style={{ padding: "6px 12px", fontSize: 13, borderRadius: 6, border: "1px solid #e2e8f0", background: "#f8fafc", color: "#334155" }}>
                  ...
                  <button onClick={() => { setShowStatement(showStatement === dealer.id ? null : dealer.id); const today = new Date().toISOString().split("T")[0]; const start = new Date(new Date().getFullYear(), 3, 1).toISOString().split("T")[0]; setStmtRange({ start, end: today }); }}
                    style={{ padding: "6px 10px", fontSize: 13, borderRadius: 6, border: "1px solid #e2e8f0", background: "#f0fdf4", color: "#166534" }}>
                    📄 Stmt
                  </button>
                </div>
                      ...
                      return (
                        <div key={bill.id} style={{ padding: "10px 16px 10px 40px", borderBottom: idx < dealer.bills.length - 1 ? "1px solid #f1f5f9" : "none", display: "flex", justifyContent: "space-between", alignItems: "center", background: "#fafbfc", flexWrap: "wrap", gap: 6 }}>
                          <div>
                            ...
                            <div style={{ fontSize: 12, color: "#888" }}>{fmtDate(bill.bill_date)}</div>
                          </div>
                          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                            <span style={{ fontFamily: "'IBM Plex Mono'", fontSize: 14, fontWeight: 500, color: "#334155" }}>{fmt(bill.balance)}</span>
                            <span style={{ fontSize: 12, padding: "2px 7px", borderRadius: 4, background: bkt.bg, color: bkt.color, border: "1px solid " + bkt.border }}>{days}d</span>
                          </div>
                        </div>
                      );
                    })}
                ...
                {showStatement === dealer.id && (
                  <div style={{ padding: "12px 16px", background: "#f0fdf4", borderTop: "1px solid #86efac" }}>
                    <div style={{ fontFamily: "'IBM Plex Mono'", fontSize: 12, letterSpacing: "0.1em", color: "#166534", marginBottom: 8 }}>DEALER STATEMENT</div>
                    <div style={{ display: "flex", gap: 8, flexWrap: "wrap", alignItems: "flex-end" }}>
                      <div style={{ flex: 1 }}>
                        <div style={{ fontFamily: "'IBM Plex Mono'", fontSize: 10, color: "#888", marginBottom: 3 }}>FROM</div>
                        <input type="date" style={{ width: "100%", padding: "8px 10px", border: "1px solid #e2e8f0", borderRadius: 6, fontSize: 14, background: "#ffffff", color: "#166534" }} value={stmtRange.start} onChange={e => setStmtRange(r => ({ ...r, start: e.target.value }))} />
                      </div>
                      <div style={{ flex: 1 }}>
                        <div style={{ fontFamily: "'IBM Plex Mono'", fontSize: 10, color: "#888", marginBottom: 3 }}>TO</div>
                        <input type="date" style={{ width: "100%", padding: "8px 10px", border: "1px solid #e2e8f0", borderRadius: 6, fontSize: 14, background: "#ffffff", color: "#166534" }} value={stmtRange.end} onChange={e => setStmtRange(r => ({ ...r, end: e.target.value }))} />
                      </div>
                      <button onClick={() => generateDealerStatement(dealer, stmtRange.start, stmtRange.end)} style={{ padding: "8px 14px", fontFamily: "'IBM Plex Mono'", fontSize: 12, borderRadius: 6, border: "none", background: "#166534", color: "#ffffff", cursor: "pointer" }}>⬇ SAVE AS PDF</button>
                      <button onClick={() => setShowStatement(null)} style={{ padding: "8px 10px", fontFamily: "'IBM Plex Mono'", fontSize: 12, borderRadius: 6, border: "1px solid #e2e8f0", background: "#ffffff", color: "#888", cursor: "pointer" }}>✕</button>
                    </div>
                  </div>
                )}
              </div>
