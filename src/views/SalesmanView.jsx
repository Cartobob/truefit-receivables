...
export default function SalesmanView({ salesman }) {
  ...
  return (
    <div className="fade-in">
      ...
      {viewingPDF && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.7)", zIndex: 1000, display: "flex", flexDirection: "column" }}>
          <div style={{ background: "#1e1c2e", padding: "12px 16px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            ...
            <button onClick={() => setViewingPDF(null)} style={{ padding: "6px 12px", fontFamily: "'IBM Plex Mono'", fontSize: 13, borderRadius: 6, border: "1px solid #444", background: "transparent", color: "#ccc", cursor: "pointer" }}>✕ Close</button>
          </div>
      ...
      {/* Cheque float */}
      {totalChq > 0 && (
        <div style={{ background: "#fffbeb", border: "1px solid #d4a820", borderRadius: 10, padding: "10px 14px", marginBottom: 14, display: "flex", alignItems: "center", gap: 8 }}>
          <span>🟡</span>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 10, marginBottom: 20 }}>
        {[{ label: "0-30 DAYS", val: b30, color: "#2d6a2d" }, { label: "31-60 DAYS", val: b60, color: "#7a4500" }, { label: "60+ DAYS", val: b60p, color: "#a32d2d" }].map(s => (
          <div key={s.label} style={{ ...card, padding: "12px 14px" }}>
            ...
            <div style={{ fontFamily: "'IBM Plex Mono'", fontSize: 15, fontWeight: 500, color: s.color }}>{fmt(s.val)}</div>
          </div>
        ))}
      </div>
                ...
                <div style={{ padding: "14px 16px" }}>
                  ...
                  {pending.length > 0 && (
                    <div style={{ marginBottom: 8, display: "flex", flexDirection: "column", gap: 3 }}>
                      {pending.map(c => (
                        <span key={c.id} style={{ background: "#fffbeb", border: "1px solid #d4a820", borderRadius: 6, padding: "2px 8px", fontSize: 13, color: "#92640a", fontFamily: "'IBM Plex Mono'", display: "inline-block" }}>
                  ...
                  {bounced.length > 0 && (
                    <div style={{ marginBottom: 8, display: "flex", flexDirection: "column", gap: 3 }}>
                      {bounced.map(c => (
                        <span key={c.id} style={{ background: "#fff5f5", border: "1px solid #fecaca", borderRadius: 6, padding: "2px 8px", fontSize: 13, color: "#dc2626", fontFamily: "'IBM Plex Mono'", display: "inline-block" }}>
