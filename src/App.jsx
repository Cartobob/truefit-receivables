...
export default function App() {
  ...
  return (
    <div style={{ fontFamily: "'IBM Plex Sans', sans-serif", background: "#f0f2f5", minHeight: "100vh", color: "#6b2f0a" }}>
      ...
      <div style={{ borderBottom: "1px solid #e2e8f0", padding: "10px 16px", display: "flex", justifyContent: "space-between", alignItems: "center", position: "sticky", top: 0, background: "#ffffff", zIndex: 100, boxShadow: "0 1px 3px rgba(0,0,0,0.06)", flexWrap: "nowrap", gap: 8 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 6, flexShrink: 0 }}>
          <div style={{ width: 7, height: 7, borderRadius: "50%", background: "#c4784a", flexShrink: 0 }} />
          <span style={{ fontFamily: "'IBM Plex Mono'", fontSize: 12, letterSpacing: "0.12em", color: "#6b2f0a", whiteSpace: "nowrap" }}>TRUEFIT SKIM COAT · RECEIVABLES</span>
          {isAdmin && (
            <span style={{ fontFamily: "'IBM Plex Mono'", fontSize: 9, padding: "2px 6px", borderRadius: 3, background: "#fdf0e8", color: "#6b2f0a", border: "1px solid #c4784a" }}>ADMIN</span>
          )}
        </div>
          ...
          {tabs.map(tab => (
            <button key={tab.id} onClick={() => setView(tab.id)}
              style={{
                ...
                background: view === tab.id ? "#6b2f0a" : "transparent",
                borderColor: view === tab.id ? "#6b2f0a" : "#e2e8f0",
                color: view === tab.id ? "#ffffff" : "#6b2f0a"
              }}>
              {tab.label}
            </button>
          ))}
